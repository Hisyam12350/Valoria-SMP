    ...
    try {
      ...
      if (["gopay", "shopeepay", "ovo", "dana"].includes(selectedMethod)) {
        ...
        if (payUrl) window.location.href = payUrl;
          ...
          router.push(
            `/payment/instruction?orderId=${data.orderId}&method=${selectedMethod}&qrUrl=${encodeURIComponent(qr)}&amount=${finalPrice}`,
          );
        } else {
          toast({
            title: "Info",
            description:
              "Silakan cek instruksi pembayaran untuk menyelesaikannya.",
          });
        }
        ...
        if (qr) {
          ...
          window.location.href = tx.pay_url;
        } else {
          toast({
            title: "Error",
            description: "Gagal memuat QR Code pembayaran.",
            variant: "destructive",
          });
        }
      } else if (["indomaret", "alfamart"].includes(selectedMethod)) {
        const paymentCode = tx.pay_code || tx.payment_code || "";
        router.push(
