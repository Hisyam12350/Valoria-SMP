        ...
        <main className="flex-1 p-6 overflow-auto">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeSection === "server-info" && (
                <ServerInfoSection admin={admin} />
              )}
              {activeSection === "staff" && <StaffSection admin={admin} />}
              {activeSection === "features" && (
                <FeaturesSection admin={admin} />
              )}
...
function OverviewSection({ admin }: { admin: AdminUser }) {
  ...
  return (
    <div>
      <SectionHeader
        title={`Selamat datang, ${admin.username}! 👋`}
        subtitle="Panel administrasi VALORIA SMP"
      />
      ...
      <Card>
        ...
        <div className="space-y-3 text-sm text-gray-400">
          <p>
            • Gunakan menu <span className="text-emerald-400">Info Server</span>{" "}
            untuk mengubah IP, port, dan link sosial
          </p>
          <p>
            • Gunakan menu <span className="text-emerald-400">Staff & Tim</span>{" "}
            untuk mengelola daftar staff
          </p>
          <p>
            • Gunakan menu{" "}
            <span className="text-emerald-400">Fitur Server</span> untuk
            mengedit kartu fitur di halaman utama
          </p>
          <p>
            • Gunakan menu{" "}
            <span className="text-emerald-400">Social & Links</span> untuk
            mengubah link Discord, WhatsApp, dll
          </p>
          {admin.role === "superadmin" && (
            <p>
              • Gunakan menu{" "}
              <span className="text-amber-400">Kelola Admin</span> untuk membuat
      ...
      <div className="mt-4 p-4 rounded-xl border border-amber-500/20 bg-amber-500/5 text-amber-300 text-sm">
        ...
        <p className="text-xs text-amber-300/70">
          Setelah mengubah data di dashboard, website akan memuat konten terbaru
          dari Supabase secara otomatis. Pastikan koneksi database aktif.
        </p>
      </div>
    </div>
  );
...
function ServerInfoSection({ admin }: { admin: AdminUser }) {
    ...
    fetch("/api/admin/content", { credentials: "include" })
      ...
      .then(({ data }) => {
        ...
        data.forEach((row: { content_key: string; content_value: unknown }) => {
          if (typeof row.content_value === "string")
            map[row.content_key] = row.content_value;
          else
            map[row.content_key] = JSON.stringify(row.content_value).replace(
              /^"|"$/g,
              "",
            );
        });
  ...
  const inputFields = [
    {
      key: "server_ip",
      label: "IP Server (Java)",
      placeholder: "play.valoriasmp.my.id",
    },
    ...
    },
    {
      key: "vote_link",
      label: "Link Vote",
      placeholder: "https://minecraft-mp.com/...",
    },
    {
      key: "server_logo",
      label: "URL Logo Server",
      placeholder: "https://...",
    },
    {
      key: "background_image",
      label: "URL Background Website",
      placeholder: "https://...",
    },
  ];
  ...
  return (
    <div>
      <SectionHeader
        title="Info Server"
        subtitle="Kelola IP, port, dan konfigurasi dasar server"
      />
      <Card>
...
function StaffSection({ admin }: { admin: AdminUser }) {
    ...
    fetch("/api/admin/content?key=staff_members", { credentials: "include" })
      ...
      .then(({ data }) => {
        if (data?.content_value) {
          setStaff(
            Array.isArray(data.content_value)
              ? data.content_value
  ...
  return (
    <div>
      <SectionHeader
        title="Staff & Tim"
        subtitle="Kelola daftar staff yang tampil di website"
      />
              ...
              <div className="flex-1 min-w-0">
                {editIdx === idx ? (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        value={member.role}
                        onChange={(e) =>
                          updateStaff(idx, "role", e.target.value)
                        }
                        className="w-full px-3 py-1.5 rounded-lg text-sm text-white outline-none mt-1"
                        style={{
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateStaff(idx, "roleColor", e.target.value)
                        }
                        className="w-full px-3 py-1.5 rounded-lg text-sm text-white outline-none mt-1"
                        style={{
                          background: "rgba(20,25,40,0.9)",
                    ...
                    <div>
                      ...
                      <input
                        value={member.skinHead}
                        onChange={(e) =>
                          updateStaff(idx, "skinHead", e.target.value)
                        }
                        className="w-full px-3 py-1.5 rounded-lg text-sm text-white outline-none mt-1"
                        style={{
                          background: "rgba(255,255,255,0.07)",
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${member.roleColor}`}>
                      {member.role}
                    </p>
                  </div>
                )}
...
function FeaturesSection({ admin }: { admin: AdminUser }) {
  ...
  const add = () =>
    setFeatures((prev) => [
      ...prev,
      { icon: "Star", title: "Fitur Baru", description: "Deskripsi fitur" },
    ]);
  ...
  return (
    <div>
      <SectionHeader
        title="Fitur Server"
        subtitle="Kelola kartu fitur yang tampil di halaman utama"
      />
      <div className="flex gap-3 mb-4">
        <button
          onClick={add}
          ...
          <Card key={idx}>
            <div className="grid grid-cols-3 gap-3 items-start">
              ...
              <div className="flex gap-2 items-start">
                ...
                <button
                  onClick={() => remove(idx)}
                  className="mt-5 p-2 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
...
function SocialSection({ admin }: { admin: AdminUser }) {
  ...
  useEffect(() => {
    fetch("/api/admin/content", { credentials: "include" })
      ...
      .then(({ data }) => {
        ...
        data.forEach((row: { content_key: string; content_value: unknown }) => {
          if (
            ["discord_link", "whatsapp_group", "vote_link"].includes(
              row.content_key,
            )
          ) {
            map[row.content_key] =
  ...
  return (
    <div>
      <SectionHeader
        title="Social & Links"
        subtitle="Kelola link sosial media dan tautan penting"
      />
      <Card>
        <div className="space-y-4">
          {items.map((i) => (
            <div key={i.key}>
              ...
              <input
                value={fields[i.key as keyof typeof fields] || ""}
                onChange={(e) =>
                  setFields((prev) => ({ ...prev, [i.key]: e.target.value }))
                }
  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle="Kelola daftar peraturan yang tampil di website"
      />
      <div className="flex gap-3 mb-4">
        <button
          onClick={() => setRules((prev) => [...prev, "Peraturan baru"])}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white"
          style={{
            background: "rgba(16,185,129,0.15)",
            border: "1px solid rgba(16,185,129,0.3)",
...
function ManageAdminsSection({ admin }: { admin: AdminUser }) {
  ...
  return (
    <div>
      <SectionHeader
        title="Kelola Admin"
        subtitle="Buat dan kelola akun admin lainnya"
      />
          ...
          {admins.map((a) => (
            <Card key={a.id}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${a.is_active ? "text-emerald-400 bg-emerald-400/10" : "text-red-400 bg-red-400/10"}`}
                  >
                    {a.is_active ? "Aktif" : "Nonaktif"}
                  </span>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${a.role === "superadmin" ? "text-amber-400 bg-amber-400/10" : "text-blue-400 bg-blue-400/10"}`}
                  >
                    {a.role}
                  </span>
                  {a.id !== admin.id && a.is_active && (
                    <button
                      onClick={() => deactivate(a.id, a.username)}
                      className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all"
                    >
    ...
    <div>
      ...
      <Card>
        <p className="text-gray-400 text-sm">
          Log aktivitas admin tersimpan di tabel{" "}
          <code className="text-emerald-400 bg-emerald-400/10 px-1 rounded">
            admin_activity_log
          </code>{" "}
          di Supabase. Lihat langsung di Supabase Dashboard → Table Editor untuk
          riwayat lengkap aktivitas admin.
        </p>
        <a
...
function TeamSection({ admin }: { admin: AdminUser }) {
  ...
  return (
    <div>
      <SectionHeader
        title="Team / Clan"
        subtitle="Kelola daftar team yang tampil di halaman Team"
      />
      <div className="flex gap-3 mb-4">
        <button
          onClick={addTeam}
              ...
              <div className="flex-1 min-w-0">
                {editIdx === idx ? (
                  <div className="grid grid-cols-2 gap-2">
                      ...
                      <div key={f.field}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateTeam(
                              idx,
                              f.field as keyof Team,
                              f.field === "activeMemberCount"
                    ...
                    <div className="col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          updateTeam(idx, "description", e.target.value)
                        }
                        rows={2}
                    ...
                    <div>
                      ...
                      <select
                        value={team.badge}
                        onChange={(e) => {
                          updateTeam(idx, "badge", e.target.value);
                          updateTeam(
                            idx,
                            "badgeColor",
                            e.target.value === "OPEN"
                  ...
                  <div>
                    <p className="text-white font-medium text-sm">
                      {team.name}{" "}
                      <span className="text-gray-500 text-xs">{team.tag}</span>
                    </p>
                    <p className="text-gray-400 text-xs">
                      Owner: {team.ownerName} · {team.activeMemberCount} member
                    </p>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full text-white mt-1 inline-block ${team.badgeColor}`}
                    >
                      {team.badge}
                    </span>
                  </div>
...
function GallerySection({ admin }: { admin: AdminUser }) {
  ...
  useEffect(() => {
    fetch("/api/admin/content?key=gallery_photos", { credentials: "include" })
      ...
      .then(({ data }) => {
        if (data?.content_value)
          setPhotos(
            Array.isArray(data.content_value) ? data.content_value : [],
          );
      })
      .finally(() => setLoading(false));
  }, []);
  ...
  const updatePhoto = (idx: number, field: keyof GalleryPhoto, value: string) =>
    setPhotos((p) =>
      p.map((ph, i) => (i === idx ? { ...ph, [field]: value } : ph)),
    );
  ...
  return (
    <div>
      <SectionHeader
        title="Galeri Foto"
        subtitle="Kelola foto-foto yang tampil di halaman Galeri"
      />
      <div className="flex gap-3 mb-4">
        <button
          onClick={addPhoto}
          ...
          <Card key={idx}>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 flex-shrink-0"
              >
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
...
function TutorialSection({ admin }: { admin: AdminUser }) {
  ...
  const removeTutorial = (idx: number) =>
    setTutorials((p) => p.filter((_, i) => i !== idx));

  const updateField = (
    idx: number,
    field: keyof TutorialItem,
    value: unknown,
  ) =>
    setTutorials((p) =>
      p.map((t, i) => {
    ...
    setTutorials((p) =>
      p.map((t, i) =>
        i === tutIdx
          ? {
              ...t,
              tips: t.tips.map((tip, ti) => (ti === tipIdx ? value : tip)),
            }
          : t,
      ),
    );
  ...
  const addTip = (tutIdx: number) =>
    setTutorials((p) =>
      p.map((t, i) => (i === tutIdx ? { ...t, tips: [...t.tips, ""] } : t)),
    );
  ...
  const removeTip = (tutIdx: number, tipIdx: number) =>
    setTutorials((p) =>
      p.map((t, i) =>
        i === tutIdx
          ? { ...t, tips: t.tips.filter((_, ti) => ti !== tipIdx) }
          : t,
      ),
    );
  ...
  return (
    <div>
      <SectionHeader
        title="Tutorial"
        subtitle="Kelola video tutorial yang tampil di halaman Tutorial"
      />
      <div className="flex gap-3 mb-5">
        <button
          onClick={addTutorial}
                ...
                {editIdx === idx ? (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <select
                          value={tut.difficulty}
                          onChange={(e) =>
                            updateField(idx, "difficulty", e.target.value)
                          }
                          className="w-full px-3 py-1.5 rounded-lg text-sm text-white outline-none"
                          style={{
                            background: "rgba(20,25,40,0.9)",
                      ...
                      <div>
                        ...
                        <select
                          value={tut.videoType}
                          onChange={(e) =>
                            updateField(idx, "videoType", e.target.value)
                          }
                          className="w-full px-3 py-1.5 rounded-lg text-sm text-white outline-none"
                          style={{
                            background: "rgba(20,25,40,0.9)",
                      ...
                      <div>
                        ...
                        <input
                          value={tut.videoId}
                          onChange={(e) =>
                            updateField(idx, "videoId", e.target.value)
                          }
                          placeholder={
                            tut.videoType === "youtube"
                              ? "dQw4w9WgXcQ"
                        ...
                        {tut.tips.map((tip, tipIdx) => (
                          <div key={tipIdx} className="flex gap-2">
                            <input
                              value={tip}
                              onChange={(e) =>
                                updateTip(idx, tipIdx, e.target.value)
                              }
                    ...
                    {tut.videoType === "youtube" && (
                      <div
                        className="p-2 rounded-lg text-xs text-gray-500"
                        style={{
                          background: "rgba(255,255,255,0.03)",
                          border: "1px solid rgba(255,255,255,0.06)",
                        }}
                      >
                        💡 Cara ambil YouTube ID: dari URL{" "}
                        <span className="text-gray-400">
                          youtube.com/watch?v=
                          <span className="text-amber-400">dQw4w9WgXcQ</span>
                        </span>
                        , bagian yang berwarna itu ID-nya
                      </div>
                    )}
                  </div>
                ) : (
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full border ${tut.categoryBg} ${tut.categoryColor}`}
                      >
                        {tut.category}
                      </span>
                      ...
                      <span className={`text-xs ${tut.difficultyColor}`}>
                        {tut.difficulty}
                      </span>
                    </div>
...
function AchievementsSection({ admin }: { admin: AdminUser }) {
  ...
  const addPhoto = (idx: number, url: string) => {
    ...
    setItems((p) =>
      p.map((a, i) =>
        i === idx ? { ...a, photos: [...a.photos, url.trim()] } : a,
      ),
    );
    setNewPhoto("");
  };
  ...
  const removePhoto = (idx: number, pIdx: number) =>
    setItems((p) =>
      p.map((a, i) =>
        i === idx
          ? { ...a, photos: a.photos.filter((_, pi) => pi !== pIdx) }
          : a,
      ),
    );
  ...
  return (
    <div>
      <SectionHeader
        title="Achievements"
        subtitle="Kelola prestasi dan pencapaian server"
      />
      <div className="flex gap-3 mb-4">
        <button
          onClick={addItem}
                ...
                {editIdx === idx ? (
                        ...
                        {item.photos.map((p, pi) => (
                          <div key={pi} className="relative group">
                            ...
                            <button
                              ...
                              className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-red-500 text-white text-xs opacity-0 group-hover:opacity-100 flex items-center justify-center"
                            >
                              ×
                            </button>
                          </div>
                  ...
                  <div>
                    ...
                    <p className="text-amber-400 text-xs">
                      {item.place} · {item.photos.length} foto
                    </p>
                  </div>
                )}
              </div>
...
function RanksSection({ admin }: { admin: AdminUser }) {
  ...
  const updateBonus = (
    idx: number,
    field: keyof Rank["bonus"],
    value: string,
  ) =>
    setRanks((p) =>
      p.map((r, i) =>
        i === idx ? { ...r, bonus: { ...r.bonus, [field]: value } } : r,
      ),
    );
  ...
  const addFeature = (idx: number) => {
    ...
    setRanks((p) =>
      p.map((r, i) =>
        i === idx ? { ...r, features: [...r.features, newFeature.trim()] } : r,
      ),
    );
    setNewFeature("");
  };
  ...
  const removeFeature = (rankIdx: number, featIdx: number) =>
    setRanks((p) =>
      p.map((r, i) =>
        i === rankIdx
          ? { ...r, features: r.features.filter((_, fi) => fi !== featIdx) }
          : r,
      ),
    );
  ...
  const save = async () => {
    ...
    try {
      ...
      notify("Gagal menyimpan ranks.", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <SectionHeader
        title="Ranks"
        subtitle="Kelola daftar rank yang dijual di halaman Store"
      />
      <div className="flex gap-3 mb-5">
        <button
          onClick={addRank}
        ...
        {ranks.map((rank, idx) => (
          <Card key={idx}>
            <div className="flex items-start gap-3">
              {/* Badge preview */}
              <div
                className={`px-3 py-1.5 rounded-lg text-xs font-bold flex-shrink-0 bg-gradient-to-r ${rank.gradient} text-white`}
              >
                {rank.name}
              </div>
              ...
              <div className="flex-1 min-w-0">
                {editIdx === idx ? (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        ...
                        <input
                          value={rank.name}
                          onChange={(e) =>
                            update(idx, "name", e.target.value.toUpperCase())
                          }
                          className="w-full px-3 py-1.5 rounded-lg text-sm text-white outline-none"
                          style={{
                            background: "rgba(255,255,255,0.07)",
                      ...
                      <div className="flex flex-wrap gap-2">
                        {RANK_COLORS.map((rc) => (
                          <button
                            key={rc.color}
                            onClick={() => {
                              update(idx, "color", rc.color);
                              update(idx, "gradient", rc.gradient);
                            }}
                        ...
                        {[
                          ...
                          <button
                            ...
                            onClick={() =>
                              update(
                                idx,
                                badge.field as keyof Rank,
                                !(rank as Record<string, unknown>)[badge.field],
                              )
                            }
                            className="px-3 py-1 rounded-lg text-xs font-medium text-white transition-all"
                            style={{
                              background: (rank as Record<string, unknown>)[
                                badge.field
                              ]
                                ? badge.color
                                : "rgba(255,255,255,0.05)",
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          ...
                          <div key={b.f}>
                            ...
                            <input
                              value={
                                (rank.bonus as Record<string, string>)[b.f] ||
                                ""
                              }
                              onChange={(e) =>
                                updateBonus(
                                  idx,
                                  b.f as keyof Rank["bonus"],
                                  e.target.value,
                                )
                              }
                    ...
                    <div>
                      <label className="text-xs text-gray-400 block mb-1">
                        Commands/Fitur ({rank.features.length})
                      </label>
                      <div className="flex flex-wrap gap-1 mb-2">
                        {rank.features.map((feat, fi) => (
                          <span
                            key={fi}
                            className="flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs text-emerald-400"
                            style={{
                    ...
                    {rank.discount > 0 && (
                      ...
                      >
                        ...
                        <span className="text-emerald-400 text-sm font-bold">
                          Rp {getPrice(rank).toLocaleString("id")}
                        </span>
  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle="Kelola daftar skill yang tersedia di store"
      />
      <Card className="mb-4">
        <div className="space-y-3">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {skills.map((skill, idx) => (
                <span
                  key={idx}
                  className="flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium text-emerald-400"
                  style={{
                    background: "rgba(16,185,129,0.1)",
                    border: "1px solid rgba(16,185,129,0.2)",
                  }}
